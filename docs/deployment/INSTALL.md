...
**Current packaged extension version:** `1.9.12-alpha` (see `crm-extension/manifest.json`)
...
The canonical release artifact is `deployment/prospecting-extension-1.9.12-alpha.zip`, with sidecar `deployment/prospecting-extension-1.9.12-alpha.zip.sha256`. The ZIP contains only `manifest.json` and `files/` (forward-slash entries). Do not include `Resources/`, `tests/`, or `custom/` placeholders.
...
| `phase3c17_provision_operational_centers_navigation.php` | Canonical C17 global navigation materializer; requires a snapshot path |
| `phase3c17_provision_sales_development_command_center.php` | One primary Sales Development Command Center dashboard for local test users |
