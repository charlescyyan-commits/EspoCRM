...
```powershell
Get-FileHash deployment\prospecting-extension-1.9.10-alpha.zip -Algorithm SHA256
```
...
| `prospecting-extension-1.9.10-alpha.zip` | Current package; matches `manifest.json` |

The current `1.9.10-alpha` ZIP has a committed `.sha256` sidecar at `deployment/prospecting-extension-1.9.10-alpha.zip.sha256`.
