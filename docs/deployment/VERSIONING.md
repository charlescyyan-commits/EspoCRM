...
|-------|---------------|----------|
| `version` | `1.9.13-alpha` | `crm-extension/manifest.json` |
| `releaseDate` | `2026-07-28` | `crm-extension/manifest.json` |
...
|----------|---------|---------|
| Extension ZIP | `prospecting-extension-<semver>.zip` | `prospecting-extension-1.9.13-alpha.zip` |
...
For the current package, the ZIP and manifest are aligned at `1.9.13-alpha` with committed sidecar `deployment/prospecting-extension-1.9.13-alpha.zip.sha256`. The frozen `1.9.12-alpha` package remains historical evidence for `phase3c19-freeze`. Build and verify from the repository root with `python crm-extension/scripts/build_release_package.py` and `python crm-extension/scripts/build_release_package.py --check` (use `py` in place of `python` on Windows when applicable).
